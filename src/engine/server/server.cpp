...
{
	if(ClientID < 0 || ClientID >= MAX_CLIENTS || m_aClients[ClientID].m_State == CServer::CClient::STATE_EMPTY || m_aClients[ClientID].m_State == CClient::STATE_BOT)
		return -1;
...
void CServer::SendServerInfo(const NETADDR *pAddr, int Token, int Type, bool SendClients)
{
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		if(m_aClients[i].m_State != CClient::STATE_EMPTY && (m_aClients[i].m_State != CClient::STATE_BOT || !m_aClients[i].m_IsClientDummy || !g_Config.m_SvHideBots))
		{
	...
	ADD_INT(p, max(MaxClients - max(g_Config.m_SvSpectatorSlots, g_Config.m_SvReservedSlots), PlayerCount)); // max players
	ADD_INT(p, ClientCount); // num clients
	ADD_INT(p, max(MaxClients - g_Config.m_SvReservedSlots, ClientCount)); // max clients
...
void CServer::RegisterCommands()
{
	...
	Console()->Register("dnsbl_status", "", CFGFLAG_SERVER, ConDnsblStatus, this, "List blacklisted players", AUTHED_ADMIN);
