...
void CGameContext::ConToTeleporter(IConsole::IResult *pResult, void *pUserData)
{
	...
	if (((CGameControllerBlockDDrace*)pSelf->m_pController)->m_TeleOuts[TeleTo-1].size())
	{
		int Num = ((CGameControllerBlockDDrace*)pSelf->m_pController)->m_TeleOuts[TeleTo-1].size();
		vec2 TelePos = ((CGameControllerBlockDDrace*)pSelf->m_pController)->m_TeleOuts[TeleTo-1][(!Num)?Num:rand() % Num];
...
void CGameContext::ConToCheckTeleporter(IConsole::IResult *pResult, void *pUserData)
{
	...
	if (((CGameControllerBlockDDrace*)pSelf->m_pController)->m_TeleCheckOuts[TeleTo-1].size())
	{
		int Num = ((CGameControllerBlockDDrace*)pSelf->m_pController)->m_TeleCheckOuts[TeleTo-1].size();
		vec2 TelePos = ((CGameControllerBlockDDrace*)pSelf->m_pController)->m_TeleCheckOuts[TeleTo-1][(!Num)?Num:rand() % Num];
		CCharacter* pChr = pSelf->GetPlayerChar(pResult->m_ClientID);
		if (pChr)
		{
			pChr->Core()->m_Pos = TelePos;
...
void CGameContext::ConForceFlagOwner(IConsole::IResult *pResult, void *pUserData)
{
	...
	((CGameControllerBlockDDrace*)pSelf->m_pController)->ForceFlagOwner(Victim, pResult->GetInteger(0));
}
...
void CGameContext::ConLaserText(IConsole::IResult *pResult, void *pUserData)
{
	...
	int Victim = pResult->NumArguments() ? pResult->GetVictim() : pResult->m_ClientID;
	CCharacter *pChr = pSelf->GetPlayerChar(Victim);
	if (pChr) pSelf->CreateLaserText(pChr->m_Pos, Victim, pResult->GetString(1));
}
