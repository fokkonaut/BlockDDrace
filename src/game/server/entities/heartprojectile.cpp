...
bool CHeartProjectile::HitCharacter()
{
	vec2 To2;
...
void CHeartProjectile::Move()
{
	m_Pos += m_Core;
	m_Core *= m_Accel;
}

void CHeartProjectile::Reset()
{
	GameServer()->m_World.DestroyEntity(this);
}

void CHeartProjectile::Tick()
{
	if (m_LifeTime == 0)
	{
		Reset();
...
void CHeartProjectile::Snap(int SnappingClient)
{
	...
	CNetObj_Pickup *pObj = static_cast<CNetObj_Pickup *>(Server()->SnapNewItem(
		NETOBJTYPE_PICKUP, m_ID, sizeof(CNetObj_Pickup)));
