...
bool CLaser::HitCharacter(vec2 From, vec2 To)
{
	...
	if (m_Type == WEAPON_SHOTGUN)
	{
		...
		if(!g_Config.m_SvOldLaser)
			Force = normalize(m_PrevPos - pHit->Core()->m_Pos) * Strength;
		else if(pOwnerChar)
			Force = normalize(pOwnerChar->Core()->m_Pos - pHit->Core()->m_Pos) * Strength;

		pHit->TakeDamage(Force, g_pData->m_Weapons.m_aId[WEAPON_SHOTGUN].m_Damage, m_Owner, WEAPON_SHOTGUN);
	}
...
void CLaser::DoBounce()
{
	...
	if(Res)
	{
		if(!HitCharacter(m_Pos, To))
		{
			...
			if (Res == TILE_TELEINWEAPON && ((CGameControllerBlockDDrace*)GameServer()->m_pController)->m_TeleOuts[z-1].size())
			{
				int Num = ((CGameControllerBlockDDrace*)GameServer()->m_pController)->m_TeleOuts[z-1].size();
				m_TelePos = ((CGameControllerBlockDDrace*)GameServer()->m_pController)->m_TeleOuts[z-1][(!Num)?Num:rand() % Num];
				m_WasTele = true;
			}
			else
			{
